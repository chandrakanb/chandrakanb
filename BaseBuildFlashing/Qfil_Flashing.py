...
def launch_qfil(command):
    ...
    converted=result.decode('utf-8')
    if "Download Fail" in converted:
        with open('D:/BaseBuildFlashing/flashing_output.txt', 'w') as f:
            f.write('Fail')
...
if len(sys.argv) != 2:
    ...
    command = fr'QFIL.exe -Mode=3 -DOWNLOADFLAT -COM={port} -SEARCHPATH="{UFSPATH}" -RawProgram="rawprogram_unsparse0.xml,rawprogram1.xml,rawprogram2.xml,rawprogram3.xml,rawprogram4.xml,rawprogram5.xml" -Patch="patch0.xml,patch1.xml,patch2.xml,patch2.xml,patch3.xml,patch4.xml,patch5.xml"'
    launch_qfil(command)
