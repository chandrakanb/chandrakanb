...
for index, row in df.iterrows():
    try:
        ...
        if result == "Fail":
            ...
            if failure_category:
                if failure_category in {"ICB Issue (Once)", "ICB Issue (Always)"}:
                    failure_category_temp = failure_category
                    failure_category = "ICB issue"
                    ...
                    failure_category = failure_category_temp                
                elif failure_category == "Human error":
                    failure_category_id = failure_category_dict[failure_category]                    
            ...
            log.info( f"    Date [Build] : {date}[{build_number}]" )
