...
status_mapping = {
    ...
    "Issue Analysis In Progress": "Analysis Completed",
    "ISSUE ANALYSIS IN PROGRESS": "Analysis Completed",
    "ISSUE ANALYSIS COMPLETED": None,
    "Issue Analysis Completed": None,  # Indicate branching happens here
    "Overnight Issue Fixed": "Sending for Review",
    "OVERNIGHT ISSUE FIXED": "Sending for Review",
    "No Fixation Required": "Sending for Review",
    "NO FIXATION REQUIRED": "Sending for Review"
...
for index, row in df.iterrows():
    try:
        ...
        fixation = row['Fixation']  # Get the Fixation value from the Excel

        issue = jira.issue(ticket_id)
        current_status = issue.fields.status.name
        if current_status != "Ready for review":
            log.info(f"Processing ticket {ticket_id} with current status: {current_status}")
        
        while current_status != "Ready for review":
            next_status_name = status_mapping.get(current_status)
            
            if next_status_name is None: # Handle branching after "Issue Analysis Completed"
                if current_status == "Issue Analysis Completed" or current_status == "ISSUE ANALYSIS COMPLETED" :
                    if fixation == "Fixed": # Overnight Issue Fixed
                        next_status_name = "Issue Fixing In Progress"
                    elif fixation == "Not Fixed": # No Fixation Required
                        next_status_name = "No Fixing Required"
                    else:
                        log.warning(f"Invalid Fixation value '{fixation}' for ticket {ticket_id}. Skipping transition.")
                        continue
                else:
                    log.info(f"No defined transition for current status: {current_status} for ticket {ticket_id}")
                    break  # No transition defined for this status
...
log.info(f"✅ JIRA TRANSITION COMPLETED...\n")