...
for index, row in df.iterrows():
    try:
        ...
        date = row['Date']
        analysis_done_by = row['Analysis Done By']
        result = row['Result']
        ...
        if ticket_id.strip():  # checks for non-empty and non-whitespace
            ...
            if result == "Fail":
                ...
                # Comment message
                comment_text = (
                    f" |  *Date* | *Analysis Done By* |  *Result*  |  *Failure Category*  |  *Failure Sub Category*  |  *Root Cause Details*  |  *Fixation Performed*  | *Remarks* | \n"
                    f" |  {date} | {analysis_done_by} |  {result}  |  {failure_category}  |  {failure_sub_category}  |  {root_cause_details}  |  {fixation_performed}  | {remarks} |"
                )
                
                # Add comment
                log.info(f"  🔄 Commenting on {ticket_id}: '{test_script_id}...")
                log.info( f"    Date                 : {date}" )
                log.info( f"    Analysis Done By     : {analysis_done_by}" )
                ...
                comment_text = (
                    ...
                    f" |  {date} | {analysis_done_by} |  {result}  | "
                )
            ...
            log.info(f"⚠️ No OVN Failure ticket is raised for script: '{test_script_id}' on {date}.\n")
