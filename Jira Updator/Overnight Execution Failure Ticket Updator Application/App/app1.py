...
class JiraConnector:
    ...
    def create_connection_ui(self):
        ...
        ttk.Checkbutton(frame, text="Save Credentials", variable=self.save_credentials_checkbox).grid(
    ...
    def create_action_selection_frame(self):
        # Create the action selection frame with radio buttons
        frame = ttk.LabelFrame(self.update_tab, text="Select Action")
        frame.pack(side='bottom', padx=10, pady=10, fill='x')


        self.action_var = tk.StringVar(value='Transition Issues')  # Default value is "Transition Issues"

        # Frame to hold the radio buttons and the Next button side by side
        action_frame = tk.Frame(frame)
        action_frame.pack(side=tk.TOP, fill='x', pady=5)

        # Create radio buttons
        options = ["Transition Issues", "Edit Issues", "Add Comment", "Change Assignee"]
        for index, option in enumerate(options):
            ttk.Radiobutton(action_frame, text=option, variable=self.action_var, value=option, command=self.toggle_action).grid(row=0, column=index, padx=5)

        # Next button next to the radio buttons
        self.action_btn = ttk.Button(
            action_frame, 
            text="Next", 
            command=lambda: (self.update_selected_ticket_frame(), self.update_selected_action_frame())
        )
        self.action_btn.grid(row=0, column=len(options), padx=5)

    def update_selected_action_frame(self):
        selected_action = self.action_var.get()
        # Dynamically update the action frame content based on selected action
        self.selected_action_label.config(text=f"Selected Action: {selected_action}")
        # Depending on the selection, you can add more UI elements to the action frames dynamically.
    ...
    def toggle_action(self):
        widgets = []
        for attr in ['action_btn', 'excel_label', 'search_entry', 'search_btn']:
        ...
        if selection == "Transition Issues":
            self.transition_action_frame()
        elif selection == "Edit Issues":
            self.edit_action_frame()
        elif selection == "Add Comment":
            self.comment_action_frame()
        elif selection == "Change Assignee":
            self.assignee_action_frame()
    ...
    def create_action_ui(self):
        ...
        # Now it's safe to call toggle_action
        self.toggle_action()
    ...
    def create_selected_action_frame(self):
        # This frame holds the selected action details that change when an action is selected
        self.selected_action_frame = ttk.LabelFrame(self.action_tab, text="Selected Action")
        self.selected_action_frame.pack(side=tk.TOP, padx=10, pady=10, fill='x')

        # Initially, the frame can be blank or show a default message
        self.selected_action_label = tk.Label(self.selected_action_frame, text="No action selected")
        self.selected_action_label.pack()
