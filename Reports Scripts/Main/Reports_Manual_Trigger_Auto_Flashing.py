...
def send_message_with_zip_to_teams(teams_webhook_url, message_string, screenshot_path):
    try:
...
if __name__ == "__main__":
    ...
    input_bench = int(9)    # input("Enter Bench Number: "))
    bench = get_bench(input_bench)
    bench_abr = bench[:-len('_TYAW')]
    ...
    screenshot_path = os.path.abspath(f"{folder_name}_Summary_Table_Pie_Chart.png")
    ...
    # Process Excel file
    pivot_excel_file_path = os.path.abspath("Execution_Report_Pivot.xlsx")
    ...
    send_message_with_zip_to_teams(teams_webhook_url, message_string, screenshot_path)
