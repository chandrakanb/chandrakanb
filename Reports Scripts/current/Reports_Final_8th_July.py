...
def send_message_to_teams(teams_webhook_url, message_parts, screenshot_path, HTML_2_3, HTML_2_4):
    try:
        # Prepare the request headers
        headers = {'Content-Type': 'application/json'}
        
        # Prepare message and attachment
        ...
        # Append each part of the message to the adaptive card content
        for idx, part in enumerate(message_parts):
            adaptive_card_content.append({
                "type": "TextBlock",
                "text": part,
                "wrap": True
            })
            
            # Add an image after the first part of the message
            if idx == 0:
                adaptive_card_content.append({
                    "type": "Image",
                    "url": f"data:image/png;base64,{encoded_file}",
                    "size": "Auto"  # Set to "Auto" to maintain original size and high quality
                })
            
            # Add HTML table content after the second and third parts of the message
            elif idx == 1:
                adaptive_card_content.append({
                    "type": "TextBlock",
                    "text": HTML_2_3,
...
if __name__ == "__main__":
    ...
    for version in ["2.3", "2.4"]:
        ...
        if version == "2.3":
            HTML_2_3 = (
                "<table border='1'>"
                f"<tr><td>Total Test Script</td><td>{test_script_total}</td></tr>"
                f"<tr><td>Pass</td><td>{test_script_pass}</td></tr>"
                f"<tr><td>Fail</td><td>{test_script_fail}</td></tr>"
                f"<tr><td>Verify Manually</td><td>{test_script_verify_manually}</td></tr>"
                "</table><br>"
            )
            # Parse HTML table using BeautifulSoup
            soup = BeautifulSoup(HTML_2_3, 'html.parser')
            # Extract table rows and data, ignoring header rows
            rows = soup.find_all('tr')[1:]  # Skip the first header row
            data = []
            for row in rows:
            ...
            df = pd.read_html(StringIO(html_string))[0]  # Wrap html_string in StringIO"""
            # Convert Pandas DataFrame to HTML Table
            HTML_2_3 = df.to_html(index=False, header=False)
            print(HTML_2_3)
        elif version == "2.4":
            HTML_2_4 = (
                "<table border='1'>"
            ...
            # Parse HTML table using BeautifulSoup
            soup = BeautifulSoup(HTML_2_4, 'html.parser')
            ...
            df = pd.read_html(StringIO(html_string))[0]  # Wrap html_string in StringIO"""
            # Convert Pandas DataFrame to HTML Table
            HTML_2_4 = df.to_html(index=False, header=False)
            print(HTML_2_4)
    ...
    if execution == overnight:
        ...
        send_message_to_teams(teams_webhook_url, message_parts, screenshot_path, HTML_2_3, HTML_2_4)
