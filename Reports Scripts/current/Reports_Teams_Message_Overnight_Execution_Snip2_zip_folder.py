...
def take_screenshot(html_file_path, screenshot_path, crop_coordinates, pivot_table_path):
    ...
    temp_screenshot_path = os.path.join(folder_name, "temp_screenshot.png")
    ...
    driver.quit()
    os.rename(temp_screenshot_path, pivot_table_path)

def create_zip(folder_path, output_path):
    """Create a zip of reports directory."""
...
def send_message_to_teams(teams_webhook_url, message_string, screenshot_path, pivot_table_path):
    try:
        ...
        with open(pivot_table_path, "rb") as f:
            encoded_pivot_table = base64.b64encode(f.read()).decode('utf-8')
        ...
        adaptive_card_content = [
            ...
            {
                "type": "Image",
                "url": f"data:image/png;base64,{encoded_pivot_table}",
                "size": "Auto"  # Set to "Auto" to maintain original size and high quality
            }
        ]
...
if __name__ == "__main__":
    ...
    screenshot_path = os.path.join(temp_folder_name, "Execution_Summary.png")
    crop_coordinates = (90, 63, 1365, 341) # crop coordinates: (left, upper, right, lower)
    pivot_table_path = os.path.join(temp_folder_name, "pivot_table.png")
    
    
    # Call Screenshot Function 
    take_screenshot(html_file_path, screenshot_path, crop_coordinates, pivot_table_path)
    ...
    # Write to Excel
    excel_file_path = os.path.join(temp_folder_name, f"{folder_name}_Execution_Report.xlsx")
    ...
    updated_excel_file_path = os.path.join(folder_name, f"{folder_name}_Execution_Report.xlsx")
    workbook.save(updated_excel_file_path)
    workbook.close() 

    # Open the Excel file and wait until the user closes the application.
    process = subprocess.Popen(['start', 'excel', updated_excel_file_path], shell=True)
    input("Press Enter if the excel is closed...")
    ...
    send_message_to_teams(teams_webhook_url, message_string, screenshot_path, pivot_table_path)
    
    # Create zip folder of reports files
    folder_path = os.path.abspath(folder_name)
    output_path = os.path.abspath(folder_name)
    create_zip(folder_path, output_path)
    os.remove(os.path.abspath(temp_folder_name))
