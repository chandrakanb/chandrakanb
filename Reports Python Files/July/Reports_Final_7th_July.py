...
def send_message_to_teams(teams_webhook_url, message, screenshot_path):
    try:
        ...
        # Prepare payload with message and attachment
        payload = {
            "text": message,
            "attachments": [
                {
                    "name": os.path.basename(screenshot_path),
                    "contentType": "image/png",  # Adjust based on your attachment type
                    "contentUrl": f"data:image/png;base64,{encoded_file}"
                }
            ]
        }
...
if __name__ == "__main__":
    ...
    for version in ["2.3", "2.4"]:
        ...
        # Flatten Column Index (Remove multi-level column index)
        ...
        if version == "2.3":
            HTML_2_3 = (
                "<table border='1'>"
                "<tr><td>Total Test Script:</td><td>{test_script_total}</td></tr>"
            ...
            HTML_2_3 = df.to_html(index=False)
            print(HTML_2_3)
        elif version == "2.4":
            HTML_2_4 = (
                "<table border='1'>"
            ...
            html_string = str(table)
            df = pd.read_html(StringIO(html_string))[0]  # Wrap html_string in StringIO
            HTML_2_4 = df.to_html(index=False)
            print(HTML_2_4)
