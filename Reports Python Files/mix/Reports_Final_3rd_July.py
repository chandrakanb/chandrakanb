...
if __name__ == "__main__":
    ...
    build = "Dev_Build" if input_build == 1 else "Rel_Build" if input_build == 2
    print('|' + '_' * 45 + '\n|\t')
    ...
    execution = overnight if flag == "1" else flashing if flag == "2"
    print('|' + '_' * 45 + '\n|\t')
    ...
    if execution == overnight:
        ...
        if input_build in [1, 2]:
            message_string = f"{date_month}{cycle} {build}{execution.replace('_', ' ')} {bench}:\n{execution_result}\n{flashing_result}"
        else:
            message_string = f"{date_month}{execution.replace('_', ' ')} {bench}:\n{execution_result}"
