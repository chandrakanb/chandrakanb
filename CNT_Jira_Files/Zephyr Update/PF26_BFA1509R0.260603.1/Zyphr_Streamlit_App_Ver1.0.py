...
AVAILABLE_FOLDERS = [
    "/PF 26/A15/BFA1509R0.260603.1/iPhone 16 Pro",
    "/PF 26/A15/BFA1509R0.260603.1/Google Pixel 9",
    "/PF 26/A15/BFA1509R0.260603.1/Samsung S25 Ultra"
]
...
JIRA_TOKEN_MAPPING = {
    "kpt_dumbre_sa" : "NDg1ODY3NjY3ODcwOlCp9+xyU32BdsJqnmj8w1pxCy1K",
    "kpt_sodnar_va" : "NzcyNDY2NzgyMTg1Op3ppp7/LfOQKJ1SxPFtYTkXqFl9",
    "kpt_subhash_sa" : "MDE3NTE2MTg1MjQ5Oqc4yGWxGSApiFdsaSE/F4GPXrwK",
    "kpt_bhalekar_ch" : "MDE3NTE2MTg1MjQ5Oqc4yGWxGSApiFdsaSE/F4GPXrwK",
    "kpt_chavan_ru" : "MzEzOTY3ODIxNjcwOg3ULfmS8AkpmdLbyXp0C03hO8NT",
    "kpt_ghadge_va" : "NDg1ODY3NjY3ODcwOlCp9+xyU32BdsJqnmj8w1pxCy1K"
}
...
def update_test_result(test_run_key, test_case_key, payload, headers):
...
def get_headers(executed_by):
    token = JIRA_TOKEN_MAPPING.get(executed_by, TOKEN)  # fallback to default TOKEN
...
def update_results(project_key, folder, build_number, excel_file, cycles):
    ...
    for idx, cycle in enumerate(cycles):
        for _, row in df.iterrows():
            executed_by = row.get("executedBy")
        ...
        for item in details.get("items", []):
            ...
            if tc in df.index:
                ...
                # Update via API
                headers = get_headers(executed_by)
                success = update_test_result(cycle_key, tc, payload, headers)
