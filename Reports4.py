...
def generate_html_table(df):
    # Start with the table tag with border CSS for all cells
    html_table = "<table border='1' style='border-collapse: collapse; width: 100%;'>"

    # Add the header row with borders on cells
    html_table += "<thead><tr>"
    for col in df.columns:
        html_table += f"<th style='border: 1px solid black; padding: 8px; text-align: center;'>{col}</th>"
    html_table += "</tr></thead>"

    # Add the data rows with borders on cells
    html_table += "<tbody>"
    for _, row in df.iterrows():
        html_table += "<tr>"
        for cell in row:
            html_table += f"<td style='border: 1px solid black; padding: 8px; text-align: center;'>{cell}</td>"
        html_table += "</tr>"
    html_table += "</tbody></table>"

    return html_table

def send_message_to_teams(teams_webhook_url, message_string, overnight_execution_screenshot_path):
    try:
...
if __name__ == "__main__":    
    ...
    if len(sys.argv) != 3 and len(sys.argv) != 4:
        sys.exit("Usage: python script_name.py <Overnight_Execution_Path> <Bench_no.> or <Build_Number>")
    else:
        Overnight_Execution_Path = Path(sys.argv[1])
        bench = f"Bench{str(sys.argv[2])}"
        if len(sys.argv) > 3:
    ...
    # parameters = [bench, build_number, execution, date, month, year]
    if build_number == "":
        parameters = [bench, date, month, year, execution]
    else:
        parameters = [bench, date, month, year, execution, build_number]
