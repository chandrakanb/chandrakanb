...
def add_file_popup():
    ...
    project_dropdown = ttk.Combobox(popup, textvariable=project_var, values=project_list, state="normal")
    if project_list:
        project_dropdown.set(project_list[0])
    project_dropdown.pack(pady=5)
    project_dropdown.focus_set()
    ...
    btn = tk.Button(popup, text="Select VNC File & Add", command=browse_and_add)
    btn.pack(pady=10)
    btn.bind("<Return>", lambda e: browse_and_add())
...
def build_project_tab(project):
    ...
    project_tabs[project] = (scroll_frame, canvas)  # store canvas along with frame

    build_bench_buttons(scroll_frame, canvas, project)

def refresh_project_tab(project):
    if project in project_tabs:
        frame, canvas = project_tabs[project]
        for widget in frame.winfo_children():
            widget.destroy()
        build_bench_buttons(frame, canvas, project)

def build_bench_buttons(frame, canvas, project):
    ...
    for i, file in enumerate(files):
        ...
        btn = tk.Button(frame, text=bench_name, width=20, takefocus=True)
        ...
        btn.bind("<Return>", lambda e, f=file: connect_bench(f))

    def on_arrow_key(event, idx):
        row = idx // columns
        col = idx % columns
        if event.keysym == "Left":
            new_idx = idx - 1 if col > 0 else idx
        elif event.keysym == "Right":
            new_idx = idx + 1 if col < columns - 1 and idx + 1 < len(buttons) else idx
        elif event.keysym == "Up":
            new_idx = idx - columns if row > 0 else idx
        elif event.keysym == "Down":
            new_idx = idx + columns if row < (len(buttons)-1)//columns else idx
        else:
            return

        if new_idx != idx:
            buttons[new_idx].focus_set()
            # Scroll canvas so button is visible:
            btn_widget = buttons[new_idx]
            # btn_widget bbox relative to canvas
            canvas.update_idletasks()
            try:
                x1, y1, x2, y2 = canvas.bbox(btn_widget)
            except Exception:
                # fallback: use winfo_rooty difference:
                y = btn_widget.winfo_rooty() - canvas.winfo_rooty()
                height = btn_widget.winfo_height()
                ...
                if y1 < scroll_y:
                    canvas.yview_moveto(y1 / canvas.bbox("all")[3])
                elif y2 > scroll_y + canvas_height:
                    canvas.yview_moveto((y2 - canvas_height) / canvas.bbox("all")[3])

    for idx, btn in enumerate(buttons):
        btn.bind("<Left>", lambda e, i=idx: on_arrow_key(e, i))
        btn.bind("<Right>", lambda e, i=idx: on_arrow_key(e, i))
        btn.bind("<Up>", lambda e, i=idx: on_arrow_key(e, i))
        btn.bind("<Down>", lambda e, i=idx: on_arrow_key(e, i))
